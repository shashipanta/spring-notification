	...
	function transformToGalleryProp(rawData: any): GalleryProp {
		...
		return {
			featuredImage: {
				...
				src: rawData?.featuredImage?.fileName
					? baseUrl + rawData.featuredImage.fileName
					: "",
			},
...
{:then apiData}
	<!-- renamed local variable to apiData -->
	{#if apiData}
		<div class="grid grid-cols-2">
			<FeaturedPropertyGallery
				featuredGalleryProp={transformToGalleryProp(apiData.data[0])}
			/>
			<pre
				class="max-h-[600px] overflow-auto text-sm bg-gray-100 p-2 rounded">{JSON.stringify(
					apiData,
					null,
					2,
				)}</pre>
